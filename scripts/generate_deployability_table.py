...
def format_sparsities(spars: List[float]) -> str:
    # Compact display to keep table narrow; spaces allow wrapping
...
def main() -> int:
    ...
    with OUT_TEX.open("w", encoding="utf-8") as f:
        ...
        # Use smaller font and tighter spacing to ensure fit
        f.write("\\begin{table}[H]\\centering\\scriptsize\n")
        f.write("\\begingroup\\setlength{\\tabcolsep}{2pt}\\renewcommand{\\arraystretch}{0.95}\n")
        f.write(
            # Model and Energy columns use tabularx's X-type (as Y via main.tex) to wrap
            "\\begin{tabularx}{\\linewidth}{@{} l r r Y r r Y @{}}\\toprule\n"
            "Model "
            "& Params "
            "& Bit "
            "& Sparsity "
        ...
        f.write("\\bottomrule\n")
        # Shorter caption; include device details here to avoid repeating per row
        caption = (
            "Deployability on {dev} ({os}, {arch}). "
            "Bit counts weights only; peak RAM assumes packed ternary weights (2 bits/param) + max float32 activations; "
            "latency is 1-sample NumPy forward (median, p95) in ms."
        ).format(dev=device.name, os=device.os, arch=device.arch)
        f.write(
